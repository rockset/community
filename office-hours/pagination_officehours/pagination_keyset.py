    ...
    def run(self):
        ...
        if 'query_errors' in self.result.keys():
            ...
            if current_doc_count > 0:
                _id = self.result['results'][-1]['_id']
                self.iterate_query(_id)
            total_time = datetime.utcnow() - self.start_time
            print("Version: {}, Total Time: {}".format(version, total_time))

    def iterate_query(self, __key):
        start_time = datetime.utcnow()
        self.parameters[2]['value'] = __key
        payload = {}
        payload['initial_paginate_response_doc_count'] = self.initialLimit
        payload['parameters'] = []
        payload['parameters'] = self.parameters
...
if __name__ == '__main__':
    ...
    parameters.append({"name": "key","type": "string","value": ""})
    processor = Processor('officehours-pagination', 'pagination_keyset', 'latest', None, parameters, None)
